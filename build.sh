...
for pdf in `ls publications/*.pdf`; do
    ...
    if [ ! -f publications/thumb_${p/pdf/png} ]; then
        ...
        magick ${pdf}[0] -density 300 -resize 150 publications/thumb_${p/pdf/png}
    fi;
done;
...
for img in `ls people/albums/*/* | grep -v 'thumb' | grep -v '.DS' `; do
    ...
    if [ ! -f ${directory}/thumb_${output_name} ]; then
        ...
        magick ${img} -auto-orient -thumbnail 200 ${directory}/thumb_${output_name}
    fi
    ...
    if [ ${image_width} -gt 1200 ]; then
        ...
        magick "${img}" -auto-orient -resize 1200 "${img}_tmp"
        mv "${img}_tmp" "${img}"
    fi
done;